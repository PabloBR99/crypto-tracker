...
export class CryptoDetailsComponent implements OnInit {
  ...
  changeChart(newScale: string): void {
    if (newScale.localeCompare('24h') != 0) {
      this.http.get(`https://api.coingecko.com/api/v3/coins/${this.crypto.id}/market_chart?vs_currency=usd&days=365&interval=daily`).subscribe((data: any) => {
        ...
        this.scale = newScale;
        if (this.scale.localeCompare('7d') == 0) {
          this.sampleData = this.sampleDataCopy.slice(-7);
          this.maxPrice = Math.max(...this.prices.slice(-8));
          this.minPrice = Math.min(...this.prices.slice(-8));
          console.log('7d: ' + this.sampleData[0].Date)
        }
        else if (this.scale.localeCompare('14d') == 0) {
          this.sampleData = this.sampleDataCopy.slice(-14);
          this.maxPrice = Math.max(...this.prices.slice(-15));
          this.minPrice = Math.min(...this.prices.slice(-15));
        } else if (this.scale.localeCompare('30d') == 0) {
          this.sampleData = this.sampleDataCopy.slice(-30);
          this.maxPrice = Math.max(...this.prices.slice(-31));
          this.minPrice = Math.min(...this.prices.slice(-31));
        } else {
  ...
  ngOnInit(): void {
    this.Activatedroute.paramMap.subscribe(params => {
      this.crypto.id = params.get('coinId')!;
    });

    // GET COIN BY ID
    this.http.get(`https://api.coingecko.com/api/v3/coins/${this.crypto.id}`).subscribe((coinById: any) => {
      this.crypto.name = coinById.name;
      this.crypto.symbol = coinById.symbol;
      ...
      this.twitter += coinById.links.twitter_screen_name;
